...
def generate_1d_contrastive_data(num_training_points, observation_noise_variance):
  ...
  observations_fg1 = (sinusoid(index_points_fg1, scale=SCALE_SHARED, shift=SHIFT_SHARED) + sinusoid(index_points_fg1, scale=SCALE_FG, shift=SHIFT_FG) +
                   np.random.normal(loc=0,
                                    scale=np.sqrt(observation_noise_variance),
                                    size=(num_training_points)))
...
# optimized_kernel_shared = tfk.ExpSinSquared(np.float64(1.), np.float64(1.), np.float64(1.))

# gprm_shared = tfd.GaussianProcess(
#       kernel=optimized_kernel_shared,
#       index_points=predictive_index_points_,
#       observation_noise_variance=np.float64(0.1))
...
# optimized_kernel_shared = tfk.ExpSinSquared(np.float64(1.), np.float64(1.), np.float64(0.1))# + tfk.ExpSinSquared(np.float64(1.), np.float64(1.), np.float64(1.))
...
# samples_shared = gprm_shared.sample(num_samples)
# for i in range(num_samples):
#   plt.plot(predictive_index_points_, samples_shared[i, :], c='b', alpha=.1,
#            label=3 if i == 0 else None)
# plt.legend()
...
def build_bg_gp(period_shared, observation_noise_variance):
  """Defines the conditional dist. of GP outputs, given kernel parameters."""

  # Create the covariance kernel, which will be shared between the prior (which we
  # use for maximum likelihood training) and the posterior (which we use for
  # posterior predictive sampling)
  kernel = tfk.ExpSinSquared(np.float64(1.), np.float64(1.), period_shared)
...
def build_fg_gp(period_shared, period_fg, observation_noise_variance):
  """Defines the conditional dist. of GP outputs, given kernel parameters."""

  # Create the covariance kernel, which will be shared between the prior (which we
  # use for maximum likelihood training) and the posterior (which we use for
  # posterior predictive sampling)
  kernel_shared = tfk.ExpSinSquared(np.float64(1.), np.float64(1.), period_shared)
  kernel_fg = tfk.ExpSinSquared(np.float64(1.), np.float64(1.), period_fg)
...
contrastive_gp_joint_model = tfd.JointDistributionNamed({
    'period_shared': tfd.LogNormal(loc=0., scale=np.float64(1.)),
    'period_fg': tfd.LogNormal(loc=0., scale=np.float64(1.)),
...
period_shared_var = tfp.util.TransformedVariable(
    initial_value=1.,
    bijector=constrain_positive,
    name='period_shared',
    dtype=np.float64)

period_fg_var = tfp.util.TransformedVariable(
    initial_value=2.,
    bijector=constrain_positive,
    name='period_fg',
    dtype=np.float64)
...
def target_log_prob(period_shared, period_fg, observation_noise_variance):
  return contrastive_gp_joint_model.log_prob({
      'period_shared': period_shared,
      'period_fg': period_fg,
      'observation_noise_variance': observation_noise_variance,
...
for i in range(num_iters):
  with tf.GradientTape() as tape:
    loss = -target_log_prob(period_shared_var, period_fg_var,
                            observation_noise_variance_var)
...
optimized_kernel_shared = tfk.ExpSinSquared(np.float64(1.), np.float64(1.), period_shared_var)
optimized_kernel_fg = tfk.ExpSinSquared(np.float64(1.), np.float64(1.), period_fg_var)
...
for i in range(num_samples):
  ...
  # plt.plot(predictive_index_points_, (samples_fg - samples_shared)[i, :], c='orange', alpha=.1,
  #          label=r'$\mathcal{GP}_fo$ posterior samples' if i == 0 else None)

...
class ContrastiveGaussianProcessRegression():
    ...
    def __init__(self, num_mixtures_shared=5, num_mixtures_fg=5):
        ...
        self.covar_module_shared = gpytorch.kernels.SpectralMixtureKernel(num_mixtures=num_mixtures_shared)
        self.covar_module_fg = gpytorch.kernels.SpectralMixtureKernel(num_mixtures=num_mixtures_fg)
        self.likelihood = gpytorch.likelihoods.GaussianLikelihood()

    class BGModel(gpytorch.models.ExactGP):
        def __init__(self, X_bg, y_bg, likelihood, covar_module):
            super(ContrastiveGaussianProcessRegression.BGModel, self).__init__(X_bg, y_bg, likelihood)
    ...
    class FGModel(gpytorch.models.ExactGP):
        def __init__(self, X_fg, y_fg, likelihood, covar_module):
            super(ContrastiveGaussianProcessRegression.FGModel, self).__init__(X_fg, y_fg, likelihood)
            self.mean_module = gpytorch.means.ConstantMean()
            self.covar_module = covar_module
    ...
    class FGOnlyModel(gpytorch.models.ExactGP):
        def __init__(self, X_fg, y_fg, likelihood, covar_module):
            super(ContrastiveGaussianProcessRegression.FGOnlyModel, self).__init__(X_fg, y_fg, likelihood)
            self.covar_module = covar_module
    ...
    def fit(self, X, y, groups, training_iter=100):
        ...
        # Define models
        self.model_bg = self.BGModel(self.X_bg, self.y_bg, self.likelihood, self.covar_module_shared)
        self.model_fg = self.FGModel(self.X_fg, self.y_fg, self.likelihood, self.covar_module_shared + self.covar_module_fg)
        self.model_fg_only = self.FGOnlyModel(self.X_fg, self.y_fg, self.likelihood, self.covar_module_fg)
        ...
        self.optimizer = torch.optim.Adam(chain(self.model_fg.parameters(), self.model_bg.parameters(), self.covar_module_shared.parameters(), self.covar_module_fg.parameters()), lr=0.1)

        # "Loss" for GPs - the marginal log likelihood
        self.mll_bg = gpytorch.mlls.ExactMarginalLogLikelihood(self.likelihood, self.model_bg)
        self.mll_fg = gpytorch.mlls.ExactMarginalLogLikelihood(self.likelihood, self.model_fg)
        self.mll_fg_only = gpytorch.mlls.ExactMarginalLogLikelihood(self.likelihood, self.model_fg_only)
        ...
        for i in range(training_iter):
            ...
            loss = -self.mll_bg(output_bg, self.y_bg) - self.mll_fg(output_fg, self.y_fg)
            loss.backward()
...
if __name__ == "__main__":
    ...
    y = np.sin(X).squeeze() + np.sin(X * 3).squeeze() * groups + np.random.normal(scale=0.1, size=n)
    model = ContrastiveGaussianProcessRegression(num_mixtures_shared=1, num_mixtures_fg=1)
